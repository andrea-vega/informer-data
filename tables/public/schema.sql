...
$function$
;
CREATE OR REPLACE FUNCTION template_processor_position()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
        BEGIN
            SELECT count(*) INTO NEW.index FROM template_processor WHERE tenant = NEW.tenant AND "templateId" = NEW."templateId";
            RETURN NEW;
        END;
    $function$
;
CREATE OR REPLACE FUNCTION ondelete_setnull_template_file_fkey()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ 
        BEGIN 
            UPDATE "template" SET "templateFileId" = NULL WHERE ("tenant", "templateFileId") IN (SELECT del."tenant", del."id" FROM "DELETED_files" del);
            RETURN NULL;
        END; 
        $function$
;
CREATE OR REPLACE FUNCTION update_new_template_timestamp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ BEGIN UPDATE template SET "updatedAt" = now() WHERE id = NEW."templateId"; RETURN NEW; END; $function$
;
CREATE OR REPLACE FUNCTION update_old_template_timestamp()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ BEGIN UPDATE template SET "updatedAt" = now() WHERE id = OLD."templateId"; RETURN OLD; END; $function$
;
CREATE OR REPLACE FUNCTION template_update_etag()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ 
         DECLARE
            etag bigint;
        BEGIN 
            SELECT nextval('template_etag') INTO etag; 
            RETURN NEW; 
        END; $function$
;
...
CREATE TABLE "comment" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "message" text  , "public" boolean DEFAULT false , "userId" character varying(255)  NOT NULL, "teamId" character varying(255)  , "mentions" character varying(255)[]  , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "_altid" integer  , "datasetId" uuid  , "datasourceId" uuid  , "reportId" uuid  , "queryId" uuid  , "jobId" uuid  , "parentId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "templateId" uuid   );
CREATE TABLE "dataset" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "ownerId" character varying(255)  , "slug" character varying(255)  , "name" character varying(255)  , "description" character varying(255)  , "embedded" boolean DEFAULT false NOT NULL, "esIndex" character varying(255)  , "esType" character varying(255) DEFAULT 'data'::character varying , "records" integer  , "size" bigint  , "source" character varying(255)  , "sourceId" jsonb  , "params" jsonb DEFAULT '{}'::jsonb , "dataUpdatedAt" timestamp with time zone  , "append" boolean DEFAULT false , "shared" boolean DEFAULT false , "timestampField" jsonb  , "lastDurationMillis" bigint DEFAULT 0 , "windowSize" integer DEFAULT 500 , "flow" jsonb[] DEFAULT '{}'::jsonb[] , "settings" jsonb DEFAULT '{}'::jsonb , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "ttl" integer  , "dataExpiresAt" timestamp with time zone  , "_altid" integer  , "datasourceId" uuid  , "reportId" uuid  , "queryId" uuid  , "editingId" uuid  , "folderId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "esId" text  , "esWorkers" integer DEFAULT 1 , "lastQueriedAt" timestamp with time zone   );
CREATE TABLE "dataset_field" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "name" character varying(255)  NOT NULL, "position" integer  , "label" character varying(255)  , "dataType" character varying(255)  , "formatOptions" jsonb  , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "_changes" jsonb  , "fieldId" character varying(255)  , "typeMapping" jsonb  , "_altid" integer  , "datasetId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "type" text DEFAULT 'field'::text , "data" jsonb  , "script" jsonb   );
...
CREATE TABLE "file" ( "id" uuid  NOT NULL, "filename" character varying(255)  , "embedded" boolean DEFAULT false NOT NULL, "contentType" character varying(255)  , "lobId" bigint  , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "ownerId" character varying(255)  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "data" jsonb DEFAULT '{}'::jsonb , "templateId" uuid  , "role" text   );
...
CREATE TABLE "report" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "type" character varying(255)  NOT NULL, "name" character varying(255)  , "ownerId" character varying(255)  NOT NULL, "slug" character varying(255)  , "description" character varying(255)  , "datasets" jsonb DEFAULT '{}'::jsonb , "defn" jsonb DEFAULT '{}'::jsonb , "settings" jsonb DEFAULT '{}'::jsonb , "shared" boolean DEFAULT false , "defnUpdatedAt" timestamp with time zone  , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "createdById" character varying(255)  , "source" character varying(255)  , "sourceId" jsonb  , "_altid" integer  , "editingId" uuid  , "datasourceId" uuid  , "folderId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL );
...
CREATE TABLE "user" ( "domain" character varying(255) DEFAULT 'local'::character varying NOT NULL, "username" character varying(255)  NOT NULL, "password" character varying(255)  , "displayName" character varying(255)  , "familyName" character varying(255)  , "givenName" character varying(255)  , "middleName" character varying(255)  , "email" character varying(255)  , "data" jsonb  , "superuser" boolean DEFAULT false NOT NULL, "settings" jsonb DEFAULT '{"log": {"log": false, "info": false, "warn": false, "debug": false, "error": true, "remote": false}}'::jsonb , "source" character varying(255)  , "sourceId" jsonb  , "lastViewedFeed" timestamp with time zone  , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "_changes" jsonb  , "enabled" boolean DEFAULT false NOT NULL, "tenant" text DEFAULT CURRENT_USER NOT NULL, "globalPermissions" jsonb DEFAULT '{"jobCreation": true}'::jsonb , "userFields" jsonb DEFAULT '{}'::jsonb  );
CREATE TABLE "user_settings" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "userId" character varying(255)  NOT NULL, "settings" jsonb DEFAULT '{}'::jsonb , "createdAt" timestamp with time zone  NOT NULL, "updatedAt" timestamp with time zone  NOT NULL, "_altid" integer  , "datasetId" uuid  , "reportId" uuid  , "queryId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "templateId" uuid   );
...
CREATE TABLE "tag_entity" ( "id" uuid DEFAULT gen_random_uuid() NOT NULL, "createdAt" timestamp with time zone  , "updatedAt" timestamp with time zone  , "_altid" integer  , "tagId" uuid  , "datasetId" uuid  , "reportId" uuid  , "queryId" uuid  , "tenant" text DEFAULT CURRENT_USER NOT NULL, "templateId" uuid   );
...
QQIDAQAB
-----END PUBLIC KEY-----
'::text NOT NULL, "license" jsonb  , "dbLogEnabled" boolean DEFAULT false , "queryLogEnabled" boolean DEFAULT false , "maxConcurrentJobs" integer DEFAULT 5 NOT NULL, "mail" jsonb DEFAULT '{}'::jsonb , "helpUrl" text DEFAULT 'https://informer5.zendesk.com/hc/en-us'::text NOT NULL, "helpEmail" text DEFAULT ''::text , "passwordStrength" "enum_system_settings_passwordStrength" DEFAULT 'Weak'::"enum_system_settings_passwordStrength" NOT NULL, "defaultDomainId" character varying(255)  , "queryDatasetTTL" integer DEFAULT 60 NOT NULL, "jobHistorySize" integer DEFAULT 200 NOT NULL, "requestLog" jsonb DEFAULT '{"api": true, "viz": true, "errors": true, "content": false, "warnings": true, "successes": true}'::jsonb , "esLogEnabled" boolean DEFAULT false , "u2BufferLogging" boolean DEFAULT false  );
...
CREATE TABLE "bundle_entry" ( "tenant" text DEFAULT CURRENT_USER NOT NULL, "id" uuid DEFAULT gen_random_uuid() NOT NULL, "bundleId" uuid  NOT NULL, "type" text  NOT NULL, "data" jsonb DEFAULT '{}'::jsonb , "datasourceId" uuid  , "queryId" uuid  , "datasetId" uuid  , "reportId" uuid  , "tagId" uuid  , "username" text  , "teamId" text  , "jobId" uuid  , "createdAt" timestamp with time zone DEFAULT now() NOT NULL, "updatedAt" timestamp with time zone DEFAULT now() NOT NULL, "templateId" uuid   );
...
CREATE TABLE "template" ( "tenant" text DEFAULT CURRENT_USER NOT NULL, "id" uuid DEFAULT gen_random_uuid() NOT NULL, "type" text  NOT NULL, "ownerId" text  NOT NULL, "name" text  , "slug" text  , "description" text  , "settings" jsonb DEFAULT '{}'::jsonb , "templateFileId" uuid  , "editingId" uuid  , "folderId" uuid  , "shared" boolean DEFAULT false , "createdAt" timestamp with time zone DEFAULT now() NOT NULL, "updatedAt" timestamp with time zone DEFAULT now() NOT NULL );
CREATE TABLE "template_processor" ( "tenant" text DEFAULT CURRENT_USER NOT NULL, "id" uuid DEFAULT gen_random_uuid() NOT NULL, "type" text  NOT NULL, "name" text  , "templateId" uuid  NOT NULL, "datasetId" uuid  , "queryId" uuid  , "datasourceId" uuid  , "index" integer  , "phase" text  , "data" jsonb DEFAULT '{}'::jsonb , "createdAt" timestamp with time zone DEFAULT now() NOT NULL, "updatedAt" timestamp with time zone DEFAULT now() NOT NULL );
CREATE TABLE "template_share" ( "tenant" text DEFAULT CURRENT_USER NOT NULL, "id" uuid DEFAULT gen_random_uuid() NOT NULL, "principalId" text  NOT NULL, "accessLevel" integer DEFAULT 1 , "createdAt" timestamp with time zone DEFAULT now() NOT NULL, "updatedAt" timestamp with time zone DEFAULT now() NOT NULL, "templateId" uuid  NOT NULL );
CREATE TABLE "input" ( "tenant" text DEFAULT CURRENT_USER NOT NULL, "id" uuid DEFAULT gen_random_uuid() NOT NULL, "dataType" text  NOT NULL, "editor" text  , "editorDefn" jsonb  , "name" text  NOT NULL, "label" text  NOT NULL, "help" text  , "index" integer  , "required" boolean  , "multiple" boolean DEFAULT false , "defaultValue" jsonb  , "templateId" uuid  , "codeId" uuid  , "fieldId" text  , "createdAt" timestamp with time zone DEFAULT now() NOT NULL, "updatedAt" timestamp with time zone DEFAULT now() NOT NULL );
...
CREATE INDEX file_template ON file USING btree (tenant, "templateId");
...
ALTER TABLE "template" ADD CONSTRAINT "template_pkey" PRIMARY KEY (tenant, id);
ALTER TABLE "template" ADD CONSTRAINT "template_tenant_ownerId_slug_key" UNIQUE (tenant, "ownerId", slug);
CREATE INDEX template_tenant ON template USING btree (tenant);
ALTER TABLE "template_processor" ADD CONSTRAINT "template_processor_pkey" PRIMARY KEY (tenant, id);
CREATE INDEX template_processor_template ON template_processor USING btree (tenant, "templateId");
CREATE INDEX template_processor_tenant ON template_processor USING btree (tenant);
ALTER TABLE "template_share" ADD CONSTRAINT "template_share_pkey" PRIMARY KEY (tenant, id);
ALTER TABLE "template_share" ADD CONSTRAINT "template_share_tenant_templateId_principalId_key" UNIQUE (tenant, "templateId", "principalId");
ALTER TABLE "input" ADD CONSTRAINT "input_pkey" PRIMARY KEY (tenant, id);
ALTER TABLE "input" ADD CONSTRAINT "input_tenant_templateId_name_key" UNIQUE (tenant, "templateId", name);
CREATE INDEX input_tenant ON input USING btree (tenant);
...
ALTER TABLE "comment" ADD CONSTRAINT "comment_templateId_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
...
ALTER TABLE "file" ADD CONSTRAINT "template_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
...
ALTER TABLE "report" ADD CONSTRAINT "createdBy" FOREIGN KEY (tenant, "createdById") REFERENCES "user"(tenant, username) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
...
ALTER TABLE "user_settings" ADD CONSTRAINT "user_settings_templateId_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE;
...
ALTER TABLE "tag_entity" ADD CONSTRAINT "tag_entity_templateId_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
...
ALTER TABLE "bundle_entry" ADD CONSTRAINT "template_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
...
ALTER TABLE "template" ADD CONSTRAINT "editing_fkey" FOREIGN KEY (tenant, "editingId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template" ADD CONSTRAINT "folder_fkey" FOREIGN KEY (tenant, "folderId") REFERENCES folder(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template" ADD CONSTRAINT "owner_fkey" FOREIGN KEY (tenant, "ownerId") REFERENCES principal(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template" ADD CONSTRAINT "template_file_fkey" FOREIGN KEY (tenant, "templateFileId") REFERENCES file(tenant, id) DEFERRABLE;
ALTER TABLE "template" ADD CONSTRAINT "template_tenant_fkey" FOREIGN KEY (tenant) REFERENCES tenant(id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_processor" ADD CONSTRAINT "dataset_fkey" FOREIGN KEY (tenant, "datasetId") REFERENCES dataset(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_processor" ADD CONSTRAINT "datasource_fkey" FOREIGN KEY (tenant, "datasourceId") REFERENCES datasource(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_processor" ADD CONSTRAINT "query_fkey" FOREIGN KEY (tenant, "queryId") REFERENCES query(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_processor" ADD CONSTRAINT "template_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_processor" ADD CONSTRAINT "template_processor_tenant_fkey" FOREIGN KEY (tenant) REFERENCES tenant(id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_share" ADD CONSTRAINT "shared_to" FOREIGN KEY (tenant, "principalId") REFERENCES principal(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_share" ADD CONSTRAINT "template_fkey" FOREIGN KEY (tenant, "templateId") REFERENCES template(tenant, id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "template_share" ADD CONSTRAINT "template_share_tenant_fkey" FOREIGN KEY (tenant) REFERENCES tenant(id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "input" ADD CONSTRAINT "code_fk" FOREIGN KEY ("codeId", tenant) REFERENCES code(id, tenant) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "input" ADD CONSTRAINT "field_fk" FOREIGN KEY ("fieldId", tenant) REFERENCES field(id, tenant) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "input" ADD CONSTRAINT "input_tenant_fkey" FOREIGN KEY (tenant) REFERENCES tenant(id) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
ALTER TABLE "input" ADD CONSTRAINT "template_fk" FOREIGN KEY ("templateId", tenant) REFERENCES template(id, tenant) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE;
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON comment FOR EACH ROW EXECUTE PROCEDURE comment_update_etag();
CREATE TRIGGER change_query_owner AFTER UPDATE ON dataset FOR EACH ROW WHEN (old."ownerId"::text IS DISTINCT FROM new."ownerId"::text) EXECUTE PROCEDURE update_query_owner();
CREATE TRIGGER delete_dataset AFTER DELETE ON dataset FOR EACH ROW EXECUTE PROCEDURE delete_orphan_datasource();
CREATE TRIGGER delete_query AFTER DELETE ON dataset FOR EACH ROW EXECUTE PROCEDURE delete_orphan_query();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON dataset FOR EACH ROW EXECUTE PROCEDURE dataset_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON dataset FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON dataset FOR EACH ROW WHEN (new.name IS NOT NULL AND (new."ownerId"::text <> old."ownerId"::text OR new.name::text <> old.name::text)) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON dataset_field FOR EACH ROW EXECUTE PROCEDURE dataset_field_update_etag();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON dataset_filter FOR EACH ROW EXECUTE PROCEDURE dataset_filter_update_etag();
CREATE TRIGGER insert_dataset_history BEFORE INSERT ON dataset_history FOR EACH ROW EXECUTE PROCEDURE insert_dataset_history();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON dataset_share FOR EACH ROW EXECUTE PROCEDURE dataset_share_update_etag();
CREATE TRIGGER update_dataset_timestamp_on_share AFTER INSERT OR UPDATE ON dataset_share FOR EACH ROW EXECUTE PROCEDURE update_new_dataset_timestamp();
CREATE TRIGGER update_dataset_timestamp_on_unshare AFTER DELETE OR UPDATE ON dataset_share FOR EACH ROW EXECUTE PROCEDURE update_old_dataset_timestamp();
CREATE TRIGGER delete_datasource AFTER DELETE ON datasource FOR EACH ROW EXECUTE PROCEDURE delete_datasource();
CREATE TRIGGER drop_workspace_schema AFTER DELETE ON datasource FOR EACH ROW WHEN (old.type::text = 'workspace'::text) EXECUTE PROCEDURE drop_workspace_schema();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON datasource FOR EACH ROW EXECUTE PROCEDURE datasource_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON datasource FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER update_embedded_datasource_idrefs AFTER UPDATE ON datasource FOR EACH ROW WHEN (old.id <> new.id) EXECUTE PROCEDURE update_query_datasource_idrefs();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON datasource FOR EACH ROW WHEN (new.name IS NOT NULL AND (new."ownerId"::text <> old."ownerId"::text OR new.name::text <> old.name::text)) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON datasource_share FOR EACH ROW EXECUTE PROCEDURE datasource_share_update_etag();
CREATE TRIGGER update_datasource_timestamp_on_share AFTER INSERT OR UPDATE ON datasource_share FOR EACH ROW EXECUTE PROCEDURE update_new_datasource_timestamp();
CREATE TRIGGER update_datasource_timestamp_on_unshare AFTER DELETE OR UPDATE ON datasource_share FOR EACH ROW EXECUTE PROCEDURE update_old_datasource_timestamp();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON field FOR EACH ROW EXECUTE PROCEDURE field_update_etag();
CREATE TRIGGER "setnull_dataset_field_fieldId_fkey" AFTER DELETE ON field REFERENCING OLD TABLE AS "DELETED_fields" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_dataset_field_fieldid_fkey();
CREATE TRIGGER update_field_id BEFORE UPDATE ON field FOR EACH ROW WHEN (old."datasourceId" <> new."datasourceId") EXECUTE PROCEDURE update_field_id();
CREATE TRIGGER delete_file_lob AFTER DELETE ON file FOR EACH ROW EXECUTE PROCEDURE delete_file_lob();
CREATE TRIGGER setnull_template_file_fkey AFTER DELETE ON file REFERENCING OLD TABLE AS "DELETED_files" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_template_file_fkey();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON link FOR EACH ROW EXECUTE PROCEDURE link_update_etag();
CREATE TRIGGER "setnull_link_parentId_fkey" AFTER DELETE ON link REFERENCING OLD TABLE AS "DELETED_links" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_link_parentid_fkey();
CREATE TRIGGER update_link_id BEFORE UPDATE ON link FOR EACH ROW WHEN (old."fromId"::text <> new."fromId"::text) EXECUTE PROCEDURE update_link_id();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON map FOR EACH ROW EXECUTE PROCEDURE map_update_etag();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON mapping FOR EACH ROW EXECUTE PROCEDURE mapping_update_etag();
CREATE TRIGGER update_mapping_id BEFORE UPDATE ON mapping FOR EACH ROW WHEN (old."datasourceId" <> new."datasourceId") EXECUTE PROCEDURE update_mapping_id();
CREATE TRIGGER update_principal_id BEFORE UPDATE ON principal FOR EACH ROW EXECUTE PROCEDURE update_principal_id();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON report FOR EACH ROW EXECUTE PROCEDURE report_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON report FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON report FOR EACH ROW WHEN (new.name IS NOT NULL AND (new."ownerId"::text <> old."ownerId"::text OR new.name::text <> old.name::text)) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER insert_report_history BEFORE INSERT ON report_history FOR EACH ROW EXECUTE PROCEDURE insert_report_history();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON report_share FOR EACH ROW EXECUTE PROCEDURE report_share_update_etag();
CREATE TRIGGER update_report_timestamp_on_share AFTER INSERT OR UPDATE ON report_share FOR EACH ROW EXECUTE PROCEDURE update_new_report_timestamp();
CREATE TRIGGER update_report_timestamp_on_unshare AFTER DELETE OR UPDATE ON report_share FOR EACH ROW EXECUTE PROCEDURE update_old_report_timestamp();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON saved_query_filter FOR EACH ROW EXECUTE PROCEDURE saved_query_filter_update_etag();
CREATE TRIGGER "setnull_mapping_setId_fkey" AFTER DELETE ON set REFERENCING OLD TABLE AS "DELETED_sets" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_mapping_setid_fkey();
CREATE TRIGGER create_principal AFTER INSERT ON team FOR EACH ROW EXECUTE PROCEDURE create_team_principal();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON team FOR EACH ROW EXECUTE PROCEDURE team_update_etag();
CREATE TRIGGER "setnull_tenant_defaultTeamId_fkey" AFTER DELETE ON team REFERENCING OLD TABLE AS "DELETED_teams" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_tenant_defaultteamid_fkey();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON team_member FOR EACH ROW EXECUTE PROCEDURE team_member_update_etag();
CREATE TRIGGER update_user_timestamp_on_member_add AFTER INSERT OR UPDATE ON team_member FOR EACH ROW EXECUTE PROCEDURE update_user_add_timestamp();
CREATE TRIGGER update_user_timestamp_on_member_remove AFTER DELETE OR UPDATE ON team_member FOR EACH ROW EXECUTE PROCEDURE update_user_remove_timestamp();
CREATE TRIGGER admin_force_enabled BEFORE UPDATE OF enabled ON "user" FOR EACH ROW WHEN (new.username::text = 'admin'::text AND new.enabled IS FALSE) EXECUTE PROCEDURE admin_force_enabled();
CREATE TRIGGER check_num_licensed_users BEFORE INSERT OR UPDATE ON "user" FOR EACH ROW EXECUTE PROCEDURE check_num_licensed_users();
CREATE TRIGGER create_principal AFTER INSERT ON "user" FOR EACH ROW EXECUTE PROCEDURE create_user_principal();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON "user" FOR EACH ROW EXECUTE PROCEDURE user_update_etag();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON visual FOR EACH ROW EXECUTE PROCEDURE visual_update_etag();
CREATE TRIGGER delete_embedded_datasource AFTER DELETE ON query FOR EACH ROW EXECUTE PROCEDURE delete_embedded_datasource();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON query FOR EACH ROW EXECUTE PROCEDURE query_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON query FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON query FOR EACH ROW WHEN (new.name IS NOT NULL AND (new."ownerId"::text <> old."ownerId"::text OR new.name::text <> old.name::text)) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER insert_query_history BEFORE INSERT ON query_history FOR EACH ROW EXECUTE PROCEDURE insert_query_history();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON query_share FOR EACH ROW EXECUTE PROCEDURE query_share_update_etag();
CREATE TRIGGER update_query_timestamp_on_share AFTER INSERT OR UPDATE ON query_share FOR EACH ROW EXECUTE PROCEDURE update_new_query_timestamp();
CREATE TRIGGER update_query_timestamp_on_unshare AFTER DELETE OR UPDATE ON query_share FOR EACH ROW EXECUTE PROCEDURE update_old_query_timestamp();
CREATE TRIGGER "setnull_tenant_defaultDomainId_fkey" AFTER DELETE ON domain REFERENCING OLD TABLE AS "DELETED_domains" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_tenant_defaultdomainid_fkey();
CREATE TRIGGER delete_expired_sessions AFTER INSERT OR UPDATE ON session FOR EACH STATEMENT EXECUTE PROCEDURE delete_expired_sessions();
CREATE TRIGGER update_session_expiration BEFORE UPDATE ON session FOR EACH ROW EXECUTE PROCEDURE update_session_expiration();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON job FOR EACH ROW EXECUTE PROCEDURE job_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON job FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON job FOR EACH ROW WHEN (new.name IS NOT NULL AND (new."ownerId"::text <> old."ownerId"::text OR new.name::text <> old.name::text)) EXECUTE PROCEDURE ensure_unique_owner_slug();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON job_history FOR EACH ROW EXECUTE PROCEDURE job_history_update_etag();
CREATE TRIGGER limit_job_history_on_insert AFTER INSERT ON job_history FOR EACH ROW EXECUTE PROCEDURE limit_job_history();
CREATE TRIGGER delete_job_dataset AFTER DELETE ON job_dataset FOR EACH ROW EXECUTE PROCEDURE delete_orphan_job_dataset();
CREATE TRIGGER delete_job_file AFTER DELETE ON job_file FOR EACH ROW EXECUTE PROCEDURE delete_orphan_job_file();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON folder FOR EACH ROW EXECUTE PROCEDURE folder_update_etag();
CREATE TRIGGER "setnull_dataset_folderId_fkey" AFTER DELETE ON folder REFERENCING OLD TABLE AS "DELETED_folders" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_dataset_folderid_fkey();
CREATE TRIGGER "setnull_query_folderId_fkey" AFTER DELETE ON folder REFERENCING OLD TABLE AS "DELETED_folders" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_query_folderid_fkey();
CREATE TRIGGER "setnull_report_folderId_fkey" AFTER DELETE ON folder REFERENCING OLD TABLE AS "DELETED_folders" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_report_folderid_fkey();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON tag FOR EACH ROW EXECUTE PROCEDURE tag_update_etag();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON tag_entity FOR EACH ROW EXECUTE PROCEDURE tag_entity_update_etag();
CREATE TRIGGER update_entity_timestamp_on_tag_add AFTER INSERT OR UPDATE ON tag_entity FOR EACH ROW EXECUTE PROCEDURE update_tag_entity_add_timestamp();
CREATE TRIGGER update_entity_timestamp_on_tag_remove AFTER DELETE OR UPDATE ON tag_entity FOR EACH ROW EXECUTE PROCEDURE update_tag_entity_remove_timestamp();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON color FOR EACH ROW EXECUTE PROCEDURE color_update_etag();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON tenant FOR EACH ROW EXECUTE PROCEDURE tenant_update_etag();
CREATE TRIGGER prune_job_histories_on_update AFTER UPDATE ON tenant FOR EACH ROW WHEN (new."jobHistorySize" < old."jobHistorySize" AND new."jobHistorySize" > 0) EXECUTE PROCEDURE prune_all_job_histories();
CREATE TRIGGER z_add_tenant_role AFTER INSERT ON tenant FOR EACH ROW EXECUTE PROCEDURE add_tenant_role();
CREATE TRIGGER z_update_tenant_role AFTER UPDATE OF id ON tenant FOR EACH ROW EXECUTE PROCEDURE update_tenant_role();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON suite FOR EACH ROW EXECUTE PROCEDURE suite_update_etag();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON suite FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_slug();
CREATE TRIGGER update_suite_slug BEFORE INSERT OR UPDATE ON suite FOR EACH ROW EXECUTE PROCEDURE update_suite_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON suite FOR EACH ROW WHEN (new.name IS NOT NULL AND new.name <> old.name) EXECUTE PROCEDURE ensure_unique_slug();
CREATE TRIGGER setnull_suite_default_partition AFTER DELETE ON suite_partition REFERENCING OLD TABLE AS "DELETED_suite_partitions" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_suite_default_partition();
CREATE TRIGGER "setnull_field_codeId_fkey" AFTER DELETE ON code REFERENCING OLD TABLE AS "DELETED_codes" FOR EACH STATEMENT EXECUTE PROCEDURE ondelete_setnull_field_codeid_fkey();
CREATE TRIGGER insert_unique_slug BEFORE INSERT ON bundle FOR EACH ROW WHEN (new.name IS NOT NULL) EXECUTE PROCEDURE ensure_unique_slug();
CREATE TRIGGER update_unique_slug BEFORE UPDATE ON bundle FOR EACH ROW WHEN (new.name IS NOT NULL AND new.name <> old.name) EXECUTE PROCEDURE ensure_unique_slug();
CREATE TRIGGER delete_user_field_value BEFORE DELETE ON user_field FOR EACH ROW EXECUTE PROCEDURE delete_user_field_data();
CREATE TRIGGER update_user_field_type AFTER UPDATE ON user_field FOR EACH ROW WHEN (old.type <> new.type) EXECUTE PROCEDURE delete_user_field_data();
CREATE TRIGGER etag_update AFTER INSERT OR DELETE OR UPDATE ON template FOR EACH ROW EXECUTE PROCEDURE template_update_etag();
CREATE TRIGGER template_processor_position BEFORE INSERT ON template_processor FOR EACH ROW WHEN (new.index IS NULL) EXECUTE PROCEDURE template_processor_position();
CREATE TRIGGER update_template_timestamp_on_share AFTER INSERT OR UPDATE ON template_share FOR EACH ROW EXECUTE PROCEDURE update_new_template_timestamp();
CREATE TRIGGER update_template_timestamp_on_unshare AFTER DELETE OR UPDATE ON template_share FOR EACH ROW EXECUTE PROCEDURE update_old_template_timestamp();
